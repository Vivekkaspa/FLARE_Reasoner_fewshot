...
class Collater_model1:
    def __init__(self):
        pass

    def __call__(self, batch):
        support_triples, support_subgraphs, support_negative_triples, support_negative_subgraphs, curr_rel = list(map(list, zip(*batch)))
        if support_subgraphs[0] is None:
            return ((torch.tensor(support_triples), None, 
                torch.tensor(support_negative_triples)), 
                curr_rel)
...
class PairSubgraphsFewShotDataLoader_model1(DataLoader):
    def __init__(
        self, dataset, batch_size: int = 1,
        shuffle: bool = False,
        **kwargs,
...
class Collater_model2:
    ...
    def __call__(self, batch):
        #print(batch)
        support_triples, support_subgraphs, support_negative_triples, support_negative_subgraphs, query_triples, query_subgraphs, negative_triples, negative_subgraphs, curr_rel = list(map(list, zip(*batch)))
        #print("gggggggg",support_subgraphs)
        return ((support_triples[0], support_subgraphs[0], 
                support_negative_triples[0], support_negative_subgraphs[0], 
                query_triples[0], query_subgraphs[0], 
                negative_triples[0], negative_subgraphs[0]), 
                curr_rel)




class PairSubgraphsFewShotDataLoader_model2(DataLoader):
    def __init__(
        self, dataset, batch_size: int = 1,
        shuffle: bool = False,
        **kwargs,
...
class SubgraphFewshotDataset(Dataset):
    def __init__(self, root, add_traspose_rels=False, shot = 1, n_query = 3, hop = 2, dataset='', mode='dev',kind = "union_prune_plus", preprocess = False, preprocess_50neg = False, skip= False, rev = False, use_fix2 = False, num_rank_negs = 50, inductive = False, orig_test = False):
        ...
        if mode == "pretrain" or mode == "pretrain2":
        ...
        if (mode == "test" or mode == "model2") and inductive:
        ...
        if mode not in ['train', 'pretrain', 'pretrain2']:
...
#         self.ssp_graph = ssp_graph
    ...
    def _save_torch_geometric(self, index):#which task
    ...
    def dict_to_torch_geometric(self, index, data_dict):
        ...
        if index == 0:
            ...
            task_index = data_dict["task_offsets"][index-1]# which sample is this task
    ...
    def __getitem__(self, index): 
        ...
        for idx, i in enumerate(curr_tasks_neg_idx):          
            ...
            if (self.mode in ["train", "pretrain", "pretrain2"] and self.dataset in ['NELL', 'FB15K-237'] and not self.inductive):
...
class SubgraphFewshotDatasetRankTail4(SubgraphFewshotDataset):
    def __len__(self):
        return len(self.eval_triples)
...
if __name__ == "__main__":
      dataset = SubgraphFewshotDataset(".", dataset="NELL", mode="test", hop = 2, shot = 2, preprocess = True)
     #dataset = SubgraphFewshotDataset(".", dataset="NELL", mode="test", hop = 2, shot = 3, preprocess = True)
    #dataset = SubgraphFewshotDataset(".", dataset="ConceptNet", mode="test", hop = 1, shot = 3, preprocess = True)
    #dataset = SubgraphFewshotDataset(".", dataset="ConceptNet", mode="test", hop = 1, shot = 3, preprocess = True)
    #pass
#     
